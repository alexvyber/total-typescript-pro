...
type ValidAmount = Brand<number, "ValidAmount">
...
const getConversionRateFromApi = async (amount: number, from: string, to: string) => {
  return Promise.resolve((amount * 0.82) as ValidAmount)
}

// Mocks a function which actually performs the conversion
const performConversion = async (user: AuthorizedUser, to: string, amount: ValidAmount) => {}

const ensureUserCanConvert = (user: User, amount: ValidAmount) => {
