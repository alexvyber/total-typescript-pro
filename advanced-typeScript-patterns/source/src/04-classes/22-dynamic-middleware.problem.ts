...
type Middleware<Input, Output> = (input: Input) => Output | Promise<Output>

/**
 * In this problem, we need to type the return type of the use()
 * method to make it update the Output generic with a new one.
 *
 * Currently, the use method just uses the same Output as the
 * first middleware you pass in. But it should infer the _new_
 * output from the middleware you pass in.
 */
class DynamicMiddleware<Input, Output> {
  private middleware: Middleware<any, any>[] = []

  constructor(firstMiddleware: Middleware<Input, Output>) {
    this.middleware.push(firstMiddleware)
  }

  // Clue: you'll need to make changes here!
  use<NewOutput>(middleware: Middleware<Output, NewOutput>): DynamicMiddleware<Input, NewOutput> {
  ...
  async run(input: Input): Promise<Output> {
    let result: Output = input as any
...
new DynamicMiddleware((req: { some: string }) => req)
  .use(req => (req.some, { ...req, other: 12 as const }))
  .use(req => (req.other, { ...req, more: false }))
  .use(req => (req.more, req))
